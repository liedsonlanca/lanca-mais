          ...
          <div className="grid gap-14 lg:grid-cols-[1fr_1.15fr] lg:gap-20">
            <Reveal>
              <dl className="space-y-8">
                {canais.map((canal) => (
                  <div key={canal.rotulo} className="border-b border-contorno pb-6">
                    <dt className="eyebrow text-tinta/55">{canal.rotulo}</dt>
                    <dd className="mt-2 text-lg text-tinta/85">{canal.valor}</dd>
                  </div>
                ))}
              </dl>

              <p className="mt-8 text-sm leading-relaxed text-tinta/62">
            ...
            <Reveal delay={0.15} distance={40}>
              <div className="rounded-3xl border border-contorno bg-cartao p-8 shadow-[0_24px_60px_-40px_rgba(10,10,8,0.35)] lg:p-10">
                <ContactForm />
              </div>
            </Reveal>
          </div>
